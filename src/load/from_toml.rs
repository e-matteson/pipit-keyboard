...
use types::{KeyPress, KmapInfo, KmapPath, ModeInfo, ModeName, Name, OpDef,
            OpType, OpVal, Options, Sequence, SwitchPos, WordInfo};
...
impl FromToml for Options {
    fn from_toml(toml_value: &Value) -> Result<Options> {
        ...
        for key in map.keys() {
            ...
            let val = map.get(key).ok_or(
                ErrorKind::MissingValue("key".into(), Some(key.to_owned())),
            )?;
            let op_val = op_def
                .value_from_toml(val)
...
impl FromToml for WordInfo {
    fn from_toml(toml_table: &Value) -> Result<WordInfo> {
        ...
        let seq_spelling =
            String::from_toml(entry.get("word").ok_or_else(
                || ErrorKind::MissingValue("word name".into(), None),
            )?)?;

        WordInfo::helper(entry, seq_spelling.clone()).chain_err(|| {
            ErrorKind::BadValue(format!("entry for '{}'", seq_spelling), None)
        })
    }
}
...
impl FromToml for SwitchPos {
    fn from_toml(toml_array: &Value) -> Result<SwitchPos> {
        ...
        if int_vec.len() != 2 {
            bail!(ErrorKind::BadValue(
                "switch position".into(),
                Some(format!("{:?}", int_vec))
            ));
        }
        Ok(SwitchPos {
            row: int_vec[0] as usize,
            col: int_vec[1] as usize,
        })
    }
}
...
impl FromToml for Vec<Vec<SwitchPos>> {
    fn from_toml(toml_array: &Value) -> Result<Vec<Vec<SwitchPos>>> {
        Ok(toml_to_vec(toml_array, |d1| {
            Ok(toml_to_vec(d1, |d2| Ok(SwitchPos::from_toml(d2)?))?)
        })?)
    }
}
    ...
    F: Fn(&Value) -> Result<T>,
{
    ...
    for (n, x) in v.iter().enumerate() {
        new_vec.push(f(x).chain_err(|| format!("at index: {}", n))?);
    }
    Ok(new_vec)
}
