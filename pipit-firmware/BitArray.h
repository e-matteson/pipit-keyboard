...
class BitArray {
  ...
  using array_t =
      std::array<T, ((NumBits + std::numeric_limits<T>::digits - 1) /
 ...
 public:
  ...
  void reset() { std::fill(blocks.begin(), blocks.end(), 0); }
  ...
  constexpr const array_t& raw() const { return blocks; }

  constexpr const T* begin() const { return blocks.begin(); }

  constexpr const T* end() const { return blocks.end(); }
 ...
 private:
  ...
  void sanitize() { blocks.back() &= mask_last_block(); }

  /// Return a mask where each bit is 1 if it used in the last block, and 0 if
  /// not.
...
/// Compare 2 BitArrays of equal length by comparing the values of their
/// underlying block types.
template <typename T, size_t NumBits>
bool operator<(const BitArray<T, NumBits>& lhs,
               const BitArray<T, NumBits>& rhs) {
  // We don't need to mask out unused bits because we assume they've already
  // been sanitized to zero.
