...
uint8_t Entry::getLength() { return length; }
...
void Entry::setLength(uint8_t value) { length = value; }

void Entry::setLastLetter(uint8_t key_code, uint8_t mod_byte) {
  last_key = Key(key_code, mod_byte);
}

Key* Entry::getLastLetter() { return &last_key; }

void Entry::setAnagrammable(bool value) { is_anagrammable = value; }

bool Entry::isAnagrammable() { return is_anagrammable; }

bool Entry::isClear() { return length == 0; }
